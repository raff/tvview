...
var (
	...
	// Implemented by our own target class, in menu_darwin.go.
...
// handled entirely inside libwebview. See observeWindowClose for why a
// notification and not a delegate.
//
...
// observeWindowClose wires windowWillClose up to the app's one real window.
// Called from installMenuBar, once target (the shared TVViewMenuTarget) and
// a.w's window both exist.
//
...
// so the quit-prompt HUD's own window (above) cannot trigger it. There is
// exactly one real window for the app's whole life — fullscreen swaps views
// inside it rather than swapping windows (see fullscreen_darwin.go) — so
// "this window closing" and "the app quitting" are the same event here.
func observeWindowClose(a *app, target objc.ID) {
